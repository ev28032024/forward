        ...
        if channel_configs:
            ...
            for channel in channel_configs:
                ...
                channel_lines.append(
                    "&nbsp;&nbsp;• Вложения: "
                    + (
                        "краткий список" if channel.formatting.attachments_style.lower() == "summary" else "список ссылок"
                    )
                )
                ...
                if any(extra_filters[name] for name in _FILTER_TYPES):
                    ...
                    if has_default_filters:
                        channel_lines.append("&nbsp;&nbsp;• Дополнительные фильтры: нет, используются глобальные")
                    else:
                        channel_lines.append("&nbsp;&nbsp;• Дополнительные фильтры: нет")
