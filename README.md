...
Для полного прогона статических анализаторов и тестов используйте:

```bash
make ci
```

Команда выполнит `ruff`, `mypy` и `pytest`. Перед публикацией убедитесь, что все проверки проходят
успешно.
